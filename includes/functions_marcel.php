<?php 
	...
	function link_software_component($Soft, $Comp, $do){
		...
		if($result == 0 && $do == "insert"){	
				$query = "INSERT INTO componenten_has_software (COMPONENT_ID, software_SOFTWARE_ID) VALUES ('{$Comp}', '{$Soft}');";
				mysqli_query($db, $query);
				$message .= "Software pakket succesvol geinstalleerd op Component {$Comp} <BR>";
		}elseif($result > 0 && $do == "insert"){
			$message .= "Software pakket ".get_software_name($Soft)." is al geinstalleerd op component {$Comp} !<BR>";
		}
		if($result > 0 && $do == "delete"){	
				$query = "DELETE FROM componenten_has_software WHERE software_SOFTWARE_ID = '{$Soft}' AND COMPONENT_ID = '{$Comp}'";
				mysqli_query($db, $query);
				$message .= "Software pakket succesvol gedeinstalleerd van Component {$Comp} <BR>";
		}
		echo $message;
	}
